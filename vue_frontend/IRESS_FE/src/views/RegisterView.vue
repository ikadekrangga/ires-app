...
const register = async () => {
  try {
    ...
    errorMessage.value = error.response?.data?.message || 'Registrasi gagal. Email mungkin sudah terdaftar.';
  } finally {
    loading.value = false;
  }
};
  ...
  <div class="min-h-screen bg-indigo-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
      ...
      <p class="mt-2 text-center text-sm text-gray-600">
        Sign up to get started with IRESS APP
      </p>
    </div>
      ...
      <div class="bg-white py-8 px-4 shadow-xl shadow-indigo-100/50 sm:rounded-2xl sm:px-10 border border-gray-100">
        ...
        <form class="space-y-6" @submit.prevent="register">
          ...
          <div>
            ...
            <p class="mt-2 text-[11px] text-gray-400">Must be at least 8 characters long.</p>
          </div>

          <div>
            <label for="password_confirmation" class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">
              Confirm Password
            </label>
            <div class="mt-1">
              <input 
